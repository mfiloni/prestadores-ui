    ...
    (function(item, spec, self, lock, ii) {
      if(!item.root || item.root === root) {
        ...
        var obj = {
          ...
          method: 'get',
          headers: {
            accept: self.scope.tuTrabajoRequestHeaders || 'application/json'
          },
          on: {
            ...
            response: function (response) {
              var tuTrabajo = response.obj;
              if (lock) {
                delete lock[item.root];
              }
              if (self.resolverCache) {
                self.resolverCache[item.root] = tuTrabajo;
              }
              self.resolveItem(tuTrabajo, item.root, resolutionTable, resolvedRefs, unresolvedRefs, item);
              processedCalls += 1;
          ...
          setTimeout(function() {
            if (lock[obj.url]) {
              ...
              if (_.isObject(cached)) {
                ...
                lock[obj.url] = true;
                new TuTrabajoHttp().execute(obj, opts);
              }
            }
          }, 0);
        })();
...
Resolver.prototype.resolveAllOf = function(spec, obj, depth) {
  ...
  for(var key in obj) {
    ...
    var item = obj[key];
    if(item === null) {
      throw new TypeError('TuTrabajo 1.0 does not support null types (' + obj + ').  no details for error.');
    }
