...
var TuTrabajoSpecConverter = require('./spec-converter');
var Q = require('q');
...
var TuTrabajoClient = module.exports = function (url, options) {
  ...
  this.tuTrabajoObject = {};
  this.deferredClient = undefined;

  this.clientAuthorizations = new auth.TuTrabajoAuthorizations();
...
TuTrabajoClient.prototype.initialize = function (url, options) {
  this.models = {};
  this.sampleModels = {};
  ...
  this.tuTrabajoRequestHeaders = options.tuTrabajoRequestHeaders || 'application/json;charset=utf-8,*/*';
...
TuTrabajoClient.prototype.build = function (mock) {
  if (this.isBuilt) {
    return this;
  }
  ...
  var obj = {
    ...
    method: 'get',
    headers: {
      accept: this.TuTrabajoRequestHeaders
    },
    on: {
      error: function (response) {
        if (self.url.substring(0, 4) !== 'http') {
          ...
          return self.fail('Can\'t read JSON from ' + self.url);
        } else {
      ...
      response: function (resp) {
        ...
        self.tuTrabajoVersion = responseObj.tuTrabajoVersion;
        self.tuTrabajoObject = responseObj;

        if (responseObj.tuTrabajo && parseInt(responseObj.tuTrabajo) === 2) {
          self.tuTrabajoVersion = responseObj.tuTrabajo;

          new Resolver().resolve(responseObj, self.url, self.buildFromSpec, self);

          self.isValid = true;
        } else {
          var converter = new tuTrabajoSpecConverter();
          self.oldtuTrabajoObject = self.tuTrabajoObject;

          converter.setDocumentationLocation(self.url);
          converter.convert(responseObj, self.clientAuthorizations, self.options, function(spec) {
            self.tuTrabajoObject = spec;
  ...
  if (this.spec) {
    self.tuTrabajoObject = this.spec;
    setTimeout(function () {
    ...
    new TuTrabajoHttp().execute(obj, this.options);
  }

  return (this.usePromise) ? this.deferredClient.promise : this;
};

TuTrabajoClient.prototype.buildFromSpec = function (response) {
  if (this.isBuilt) {
    return this;
  }
    ...
    _.forEach(supportedOperationMethods, function (method) {
      ...
      _.forEach(tags, function (tag) {
        ...
        if (clientProperty !== tag) {
          helpers.log('The \'' + tag + '\' tag conflicts with a TuTrabajoClient function/property name.  Use \'client.' +
                      clientProperty + '\' or \'client.apis.' + tag + '\' instead of \'client.' + tag + '\'.');
        }

        if (apiProperty !== tag) {
          helpers.log('The \'' + tag + '\' tag conflicts with a TuTrabajoClient operation function/property name.  Use ' +
                      '\'client.apis.' + apiProperty + '\' instead of \'client.apis.' + tag + '\'.');
        }

        if (_.indexOf(reservedApiTags, operationId) > -1) {
          helpers.log('The \'' + operationId + '\' operationId conflicts with a TuTrabajoClient operation ' +
...
TuTrabajoClient.prototype.makeUniqueOperationId = function(operationId, api) {
  var count = 0;
...
TuTrabajoClient.prototype.parseUri = function (uri) {
  var urlParseRE = /^(((([^:\/#\?]+:)?(?:(\/\/)((?:(([^:@\/#\?]+)(?:\:([^:@\/#\?]+))?)@)?(([^:\/#\?\]\[]+|\[[^\/\]@#?]+\])(?:\:([0-9]+))?))?)?)?((\/?(?:[^\/\?#]+\/+)*)([^\?#]*)))?(\?[^#]+)?)(#.*)?/;
...
TuTrabajoClient.prototype.help = function (dontPrint) {
  var output = '';

  if (this instanceof TuTrabajoClient) {
...
TuTrabajoClient.prototype.tagFromLabel = function (label) {
  return label;
};

TuTrabajoClient.prototype.idFromOp = function (path, httpMethod, op) {
...
TuTrabajoClient.prototype.setHost = function (host) {
  this.host = host;
...
TuTrabajoClient.prototype.setBasePath = function (basePath) {
...
TuTrabajoClient.prototype.setSchemes = function (schemes) {
...
TuTrabajoClient.prototype.fail = function (message) {
  if (this.usePromise) {
